...
namespace Shared.Communication
{
    ...
    public class Packet : IDisposable
    {
        ...
        /// <summary>Reads a Dictionary<RessourceType, bool> from the packet.</summary>
        /// <param name="_moveReadPos">Whether or not to move the buffer's read position.</param>
        public Feedback ReadFeedback(bool _moveReadPos = true)
        {
            Feedback feedback = new Feedback();
            feedback.feedbackStyle = (Feedback.FeedbackStyle)ReadByte(_moveReadPos);
            feedback.successfull = ReadBool(_moveReadPos);

            switch (feedback.feedbackStyle)
            {
                case Feedback.FeedbackStyle.harvest:
                    feedback.resource = (RessourceType)ReadByte(_moveReadPos);
                    feedback.quantity = ReadInt(_moveReadPos);
                    break;
                case Feedback.FeedbackStyle.build:
                    feedback.message=ReadString(_moveReadPos);
                    break;
                case Feedback.FeedbackStyle.upgrade:
                    feedback.message = ReadString(_moveReadPos);
                    feedback.type = ReadType(_moveReadPos);
                    break;
                case Feedback.FeedbackStyle.destroy:
                    feedback.quantity = ReadInt(_moveReadPos);
                    feedback.type = ReadType(_moveReadPos);
                ...
                case Feedback.FeedbackStyle.battle:
                    ...
                    if (feedback.successfull)
                    {
                        feedback.battleLog = new Feedback.BattleLog();
                        feedback.battleLog.attackerTroops = ReadDictionaryTroopTypeInt(_moveReadPos);
                        feedback.battleLog.defenderTroops = ReadDictionaryTroopTypeInt(_moveReadPos);
                        feedback.battleLog.winnersRest = ReadDictionaryTroopTypeInt(_moveReadPos);
                        feedback.battleLog.result = (Feedback.BattleLog.BattleScore)ReadByte(_moveReadPos);
                        feedback.type = ReadType(_moveReadPos);
                        feedback.TribeId = ReadByte(_moveReadPos);
                        feedback.coordinates = ReadHexCoordinates(_moveReadPos);
                    }
